...
export function MainLayout({ children, breadcrumbs }: MainLayoutProps) {
  return (
    <div className="min-h-screen bg-[var(--agyn-bg-light)] flex flex-col">
      <header className="flex h-14 items-center justify-between border-b border-[var(--agyn-border-subtle)] bg-white px-6">
        <div className="flex items-center gap-4">
          <Link to="/" className="flex items-center" aria-label="Agyn Tracing home">
            <LogoMark />
          </Link>
          <div className="h-5 w-px bg-[var(--agyn-border-subtle)]" aria-hidden="true" />
          <nav className="flex items-center text-sm text-[var(--agyn-text-subtle)]" aria-label="Breadcrumbs">
            {breadcrumbs.map((item, index) => {
              const isLast = index === breadcrumbs.length - 1;
              const content = item.to && !isLast ? (
                <Link to={item.to} className="transition-colors hover:text-[var(--agyn-blue)]">
                  {item.label}
                </Link>
              ) : (
                <span className="font-medium text-[var(--agyn-dark)]">{item.label}</span>
              );
              return (
                <div key={`${item.label}-${index}`} className="flex items-center">
                  {content}
                  {!isLast && <ChevronRight className="mx-2 h-4 w-4 text-[var(--agyn-border-subtle)]" />}
                </div>
              );
            })}
          </nav>
